...
class SimuladorMIPS32:
    ...
    def executar_instrucao(self, instrucao):
        ...
        if opcode == 'add':
            ...
            try:
                ...
                endereco = endereco[:-1]  # Remove o ')' do final
                offset = int(offset)
                if endereco in self.vetores:
                    # Verifica se o vetor é de palavras (.word)
                    if isinstance(self.vetores[endereco], list):
                        indice = offset // 4  # Assume que cada palavra ocupa 4 bytes
    ...
    def obter_saidas(self):
        return '\n'.join(map(str, self.saidas))  # Converte inteiros para strings antes de unir
